...
img = cv2.imread("./img/xm6.jpg")
cv2.imshow("Original image", img)
(himg,wimg,chanel)=img.shape
#ảnh nào cũng như nhau 800x600
#nếu muốn nhận diện biển xe máy thì mở dòng 26,35,36
# img=cv2.resize(img,dsize=(1000,700))
#tiền xử lí ảnh
grayImg = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
#khử hạt cân bằng histogram
if(wimg<600 and himg <500):
    if(wimg/himg >2):
        noise_removal = cv2.bilateralFilter(grayImg,9,75,75)
        ret, binImg = cv2.threshold(noise_removal, 100, 255, cv2.THRESH_BINARY_INV)
    else:
        equal_histogram = cv2.equalizeHist(grayImg)
        noise_removal = cv2.bilateralFilter(equal_histogram,9,75,75)
        # ret, binImg = cv2.threshold(equal_histogram, 100, 255, cv2.THRESH_BINARY_INV)
        ret, binImg = cv2.threshold(noise_removal, 100, 255, cv2.THRESH_BINARY_INV)
        #tạo stroke cho ảnh (rộng,dài)
        kerel3 = cv2.getStructuringElement(cv2.MORPH_RECT,(1,1))
        binImg = cv2.morphologyEx(binImg,cv2.MORPH_DILATE,kerel3)
elif wimg >1000 and himg>1000:
        noise_removal = cv2.bilateralFilter(grayImg,9,75,75)
        ret, binImg = cv2.threshold(noise_removal, 100, 255, cv2.THRESH_BINARY_INV)
else:
    equal_histogram = cv2.equalizeHist(grayImg)
    ...
    #tạo stroke cho ảnh (rộng,dài)
    kerel3 = cv2.getStructuringElement(cv2.MORPH_RECT,(10,1))
    binImg = cv2.morphologyEx(binImg,cv2.MORPH_DILATE,kerel3)
...
for c in cnts:
    ...
    white = cv2.countNonZero(binImg)
    rationwhite=white/(h*w);
    if (h/w >1.5 and h/w <4 and cv2.contourArea(c)>100):#and h > himg*0.5:
